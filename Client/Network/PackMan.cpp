...
static std::map<eRequestType, IRequest* (*)(TCPPacket*)>	init_request_tab()
{
	...
	map.insert(std::make_pair(ANSWER_CREATE_ROOM, &PackMan::unpack_T<AnswerCreateRoom>));
	map.insert(std::make_pair(CLIENT_INVITED, &PackMan::unpack_T<ClientInvited>));
	map.insert(std::make_pair(GAME_LAUNCHED, &PackMan::unpack_T<GameLaunched>));
	map.insert(std::make_pair(ACK_, &PackMan::unpack_T<ACK>));
